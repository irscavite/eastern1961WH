...
function getVisibleConsumableSummary() {
  ...
  return searchTerm ? rows : rows.slice(0, 10);
}
...
function renderConsumables() {
  ...
  }).join("") : `<tr><td class="empty-row" colspan="5">No consumable inventory yet. Click “New Item / Beginning Stock” to create the first stock entry.</td></tr>`;
  ...
  consumableTable.innerHTML = filtered.slice().reverse().slice(0,10).map(record => `
    <tr>
...
if (consumableForm) {
  consumableForm.addEventListener("submit", async function (event) {
    ...
    if (qty <= 0) {
      alert("Remaining Stock must be greater than zero.");
      return;
    }
...
if (exportConsumablesBtn) {
  exportConsumablesBtn.addEventListener("click", function () {
    ...
    const headers = ["Date", "Item Name", "Last PR Date", "Transaction Type", "Remaining Stock", "Unit", "Requested / Received By", "NHJ / DR Number", "Remarks"];
