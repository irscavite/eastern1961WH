...
function setConsumableTransactionType(type) {
  ...
  if (consumableTypeDisplay) {
    ...
    consumableTypeDisplay.className = `readonly-pill ${safeType === "Withdrawal" ? "pill-withdrawal" : "pill-replenishment"}`;
  }
  if (consumableFormTitle) consumableFormTitle.textContent = safeType === "Withdrawal" ? "Withdraw Consumable Item" : "Replenish Consumable Item";
  const subtitle = document.getElementById("consumableFormSubtitle");
  if (subtitle) subtitle.textContent = safeType === "Withdrawal" ? "Stock-out transaction. This will deduct from current stock." : "Stock-in transaction. This will add to current stock.";
  if (saveConsumableBtn) saveConsumableBtn.textContent = safeType === "Withdrawal" ? "Save Withdrawal" : "Save Replenishment";
}
...
function getConsumableSummary() {
  ...
  return Array.from(summary.values()).map(row => ({
    ...row,
    stock: row.replenishment - row.withdrawal
  })).sort((a, b) => a.itemName.localeCompare(b.itemName));
}

function renderConsumables() {
  if (!consumableTable || !consumableSummaryTable) return;

  const filtered = getFilteredConsumables();
  if (consumableCount) consumableCount.textContent = `Total Transactions: ${filtered.length}`;

  const visibleSummary = getConsumableSummary();
  ...
  consumableSummaryTable.innerHTML = visibleSummary.length ? visibleSummary.map(row => {
    ...
    return `
      <tr>
        ...
        <td>${escapeHTML(row.replenishment)}</td>
        <td>${escapeHTML(row.withdrawal)}</td>
        ...
        <td class="action-cell">
          ...
          <button class="warning-btn" onclick="openConsumableTransaction('Withdrawal', '${itemData}')">Withdrawal</button>
        </td>
      </tr>`;
  ...
  if (filtered.length === 0) {
    consumableTable.innerHTML = `<tr><td class="empty-row" colspan="10">No transaction history found.</td></tr>`;
    return;
  }

  consumableTable.innerHTML = filtered.slice().reverse().map(record => `
    <tr>
      <td>${escapeHTML(record.date || "")}</td>
      <td>${escapeHTML(record.itemName || "")}</td>
      <td>${escapeHTML(record.category || "-")}</td>
      <td>${getConsumableTypeBadge(record.transactionType)}</td>
      <td>${escapeHTML(record.qty || "")}</td>
      <td>${escapeHTML(record.unit || "")}</td>
      ...
      <td>${escapeHTML(record.remarks || "-")}</td>
      <td>
        <button class="edit-btn" onclick="editConsumable('${record.id}')">Edit</button>
        <button class="danger-btn" onclick="deleteConsumable('${record.id}')">Delete</button>
      </td>
    </tr>
  `).join("");
}
...
if (exportConsumablesBtn) {
  exportConsumablesBtn.addEventListener("click", function () {
    if (motorpoolConsumables.length === 0) {
      alert("No consumable transactions to export.");
      return;
    }

    const headers = ["Date", "Item Name", "Category", "Transaction Type", "QTY", "Unit", "Requested / Received By", "Reference No", "Remarks"];
    const rows = motorpoolConsumables.map(record => [
      record.date || "",
      record.itemName || "",
      record.category || "",
    ...
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "motorpool_consumables_history.csv";
    link.click();
  });
}
